---
title: "Cosmic Latte – The Average Color of the Universe"
date: "2026-03-24"
datetime: "2026-03-24 11:30:27"
id: "41634"
slug: "cosmic-latte-the-average-color-of-the-universe"
url: "https://eay.cc/2026/cosmic-latte-the-average-color-of-the-universe/"
author: "eay"
format: "link"
categories:
  - 0815
tags:
  - farben
  - weltall
  - wikipedia
  - wissenschaft
meta:
  - linked_list_url: "https://en.wikipedia.org/wiki/Cosmic_latte"
  - geo_latitude: "51.000098"
  - geo_longitude: "6.794167"
  - geo_public: "1"
  - linked_list_og-title: "Cosmic latte - Wikipedia"
  - linked_list_og-description: "Finding the average color of the universe was not the focus of the study. Rather, the study examined spectral analysis of different galaxies to study star formation."
  - linked_list_og-image: "https://eay.cc/wordpress/uploads/og-cache/b8df3e606a8fa24b71970f6a1b227d0c_upload.webp"
  - yourls_shorturl: "https://eay.li/40j"
  - _share_on_mastodon_url: "https://eay.social/@eay/116283685612960409"
  - content_copy: "https://bsky.app/profile/eay.social/post/3mhseqva46p2k"
---

![Die Farbe Cosmic Late als Bild.](https://eay.cc/uploads/2026/cosmic-latte.png)

> Cosmic latte is a term coined by a team of astronomers from Johns Hopkins University to describe the average color of the galaxies of the universe as perceived by a typical human observer from the position of the Earth.

> In 2002, Karl Glazebrook and Ivan Baldry determined that the average color of the universe was a whiteish green. Still, they soon corrected their analysis in a 2003 paper in which they reported that their survey of the light from over 200,000 galaxies averaged to a whiteish beige. The hex triplet value (assuming the standard sRGB color space) for the cosmic latte color is approximately `#FFF8E7`.

(via meinem Tweet-Archiv, das ich letztens in [Everlog](https://everlog.app/), meine Journaling-App der Wahl, importiert habe und seitdem immer wieder auf interessante, alte Tweets stoße; den Hinweis hierauf habe ich z.B. 2013 getwittert)
